...
exports.updateData = function(req, res) {
    ...
    if (!jobdata) return res.send({ error: 'invalid job data' });
    Job.get(id, function (err, job) {
        if (err) return res.send({ error: err.message });
        // reset or append the data to job.data.
        for(var p in jobdata)
            job.data[p] = jobdata[p];
        job.progress((jobdata.success + jobdata.fail), jobdata.count);
        if(jobdata.success + jobdata.fail === jobdata.count) // complete
        {
            if(jobdata.fail === 0)
                job.complete();
            else
        ...
        job.save(function (err) {
            if (err) return res.send({ error: err.message });
            console.log('updated the job ' + id + ' with data - success: ' + jobdata.success + 
                ', fail: ' + jobdata.fail + ', count: ' + jobdata.count + '.');
            res.send({ message: 'updated job data' });
        });
    });
...
exports.writelog = function(req, res) {
    ...
    Job.get(id, function(err, job){
        ...
        job.log(JSON.stringify(logdata));

        job.save(function(err) {
            if (err) return res.send({ error: err.message });
            res.send({ message: 'updated job log' });
        })
    });
}
